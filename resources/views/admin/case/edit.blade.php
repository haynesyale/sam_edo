...
@section('content')
    ...
    <div class="panel panel-default">
        <ul class="nav nav-tabs" role="tablist">
            ...
            <li class="active"><a href="/admin/case/create">编辑案例</a></li>
        </ul>
        <div class="panel-body">
            <form action="/admin/case/{{$data['id']}}" method="post" class="form-horizontal" role="form">
                {{csrf_field()}}
                {{method_field('PUT')}}
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-10">
                        <input type="text" name="bname" class="form-control" value="{{$data['bname']}}">
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-sm-10">
                        <select name="tid" class="form-control">
                            ...
                            @foreach(\App\Model\Trade::get() as $v)
                                @if($v['id']==$data['tid'])
                                <option value="{{$v['id']}}" selected="selected">{{$v['tname']}}</option>
                                @else
                        ...
                        @foreach(\App\Model\Cate::where('pid',0)->get() as $v)
                            @if(in_array($v['id'],$cates))
                                <label class="checkbox-inline col-sm-2" >
                                    <input type="checkbox"   checked class="cate" name="cid[]" value="{{$v['id']}}"> {{$v['cname']}}
                                </label>
                                @else
                                <label class="checkbox-inline col-sm-2" >
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-10">
                        <input type="number"  name="zan" placeholder="必须是整数" class="form-control"  value="{{$data['zan']}}">
                    </div>
                </div>
                {{--中图上传--}}
                <style>
                ...
                <div class="form-group">
                    <div class="col-sm-10">
                        ...
                        <div class="col-sm-8">
                            <input type="text" name="preview" id="mpic" class="form-control" value="{{$data['preview']}}" readonly>
                        </div>
                    </div>
                </div>
                <div class="form-group">
                ...
                <div class="form-group col-sm-10">
                    <div id="box1">
                        @foreach( explode('|',$data['preview']) as $v)
                            @if($v!='')
                                <img src="{{$v}}" alt="">
                            @endif
                        @endforeach
